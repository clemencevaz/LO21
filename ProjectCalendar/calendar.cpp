...
void  TacheManager::save(const string& f){
	file=f;
	ofstream fout(f,ofstream::trunc); // toutes les taches existantes sont écrasées
	for(unsigned int i=0; i<nb; i++){
		fout<<*taches[i];
	}
	fout.close();
...
/*QUESTION 1:
1re partie sans encapsulation dans Handler_______________________________
TacheManager* TacheManager::instance=0;

TacheManager& TacheManager:: getInstance(){
    if (instance==0) instance = new TacheManager;
    return * instance;
}
...
2me partie avec encapsulation dans Handler_______________________________*/
//
...
void TacheManager::libererInstance(){
    std::cout<<"appel de lib instance \n";
    system("pause");
